...
if ffi.os == "Windows" then
	cdecl = cdecl:gsub("APIENTRYP", "__stdcall *")
	cdecl = cdecl:gsub("APIENTRY", "__stdcall")
else
	cdecl = cdecl:gsub("APIENTRYP", "*")
...
function functionAddress(tbl, key)
	local cName
	if key:sub(1, 2) == "gl" then
		cName = key
	else
		cName = "gl" .. key
	end
	local procName = "PFN" .. cName:upper() .. "PROC"
	local procAdr = glfw.GetProcAddress(cName)
	local func
	if procAdr then func = ffi.cast(procName, procAdr) end
	if not func then
		print("OpenGL -> Invalid function address, bad luck mate;\n\t", key, " doesn't exit within OpenGL.")
	return end
	tbl[key] = func
	tbl[cName] = func
	return func
...
setmetatable(gl, {__index = functionAddress})
